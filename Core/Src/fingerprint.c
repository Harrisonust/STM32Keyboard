...
void receive(){
	...
	send(fpacket);
	HAL_UART_Receive (&UART_HANDLER, testing, 100, 1000); //logically hitting all the 9 data
	setup_received(testing);

//	//get data and checksum
//	HAL_UART_Receive(&UART_HANDLER, &(rpacket->data), rpacket->length - 2 , timeout);
...
uint8_t save_fingerprint(uint8_t id){
	...
	if(rpacket.data[0]) return rpacket.data[0];
